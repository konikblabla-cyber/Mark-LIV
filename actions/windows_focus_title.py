...
def windows_focus_title(parameters=None, **kwargs):
    ...
    p = parameters or {}
    needle = str(p.get("contains") or "").strip().lower()
    if not needle:
    ...
    if len(matches) != 1:
        return f"Expected exactly one matching window; found {len(matches)}."
    u.SetForegroundWindow(matches[0])
    return f"Focused window matching '{needle}'."
