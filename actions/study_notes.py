...
def study_notes(parameters=None,**kwargs):
 ...
 if a=="remove":
  ...
  if k in d: del d[k];FILE.write_text(json.dumps(d,ensure_ascii=False,indent=2),encoding="utf-8");return f"Removed note: {k}"
  return f"No note for: {k}"
 return "\n".join(f"- {k}: {v}" for k,v in d.items()) or "No study notes."
