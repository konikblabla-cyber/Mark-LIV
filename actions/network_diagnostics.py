...
def network_diagnostics(parameters=None,**kwargs):
    ...
    p=parameters or {}
    a=str(p.get("action","quick")).lower().strip()
    cmds={"quick":["ipconfig","/flushdns"],"config":["ipconfig","/all"],"routes":["route","print"],"dns":["nslookup","example.com"],"arp":["arp","-a"]}
    if a not in cmds:
        return "Use quick, config, routes, dns or arp."
    c=cmds[a]
    try:
        r=subprocess.run(c,capture_output=True,text=True,timeout=20,creationflags=subprocess.CREATE_NO_WINDOW)
