...
def windows_file_event_watch(parameters=None, **kwargs):
    ...
    p = parameters or {}
    root = str(p.get("path", "")).strip()
    try:
        seconds = max(0.5, min(float(p.get("seconds", 10)), 30))
    except (TypeError, ValueError):
        return "Invalid watch duration."
    if not root:
        return "Directory path is required."
    safe_root = root.replace("'", "''")
    ps = (
        "$p=Join-Path $env:TEMP 'markliv_watch_$([guid]::NewGuid().ToString()).log'; "
        "$w=New-Object IO.FileSystemWatcher; "
        f"$w.Path='{safe_root}'; "
        "$w.IncludeSubdirectories=$true; $w.EnableRaisingEvents=$true; "
        "$a={Add-Content $p ('{0}|' -f $Event.SourceEventArgs.ChangeType + $Event.SourceEventArgs.FullPath)}; "
        "Register-ObjectEvent $w Created -Action $a | Out-Null; "
        "Register-ObjectEvent $w Changed -Action $a | Out-Null; "
        "Register-ObjectEvent $w Deleted -Action $a | Out-Null; "
        "Register-ObjectEvent $w Renamed -Action $a | Out-Null; "
        f"Start-Sleep -Seconds {seconds}; "
        "Get-Content $p -ErrorAction SilentlyContinue; "
        "Remove-Item $p -Force -ErrorAction SilentlyContinue; $w.Dispose()"
    )
    r = subprocess.run(["powershell.exe","-NoProfile","-NonInteractive","-Command",ps], capture_output=True, text=True, timeout=seconds+8, creationflags=getattr(subprocess,"CREATE_NO_WINDOW",0))
