...
def windows_idle_time(parameters=None,**kwargs):
 u=ctypes.windll.user32;i=LI();i.cbSize=ctypes.sizeof(LI)
 if not u.GetLastInputInfo(ctypes.byref(i)):return "Unable to read idle time."
 tick=ctypes.windll.kernel32.GetTickCount();sec=max(0,(tick-i.dwTime)&0xffffffff)/1000
